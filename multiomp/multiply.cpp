...
void multmatomp(int **A, int **B, int **C, int N,int time);
...
void multmatomp(int **A, int **B, int **C, int N,int time){
    ...
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
    cout << "in Time of execute withomp it "<< ": "<< duration.count() <<" microsegundos" <<endl <<endl;
    time = duration.count();
}
